...
const MemberCenter: React.FC = () => {
  ...
  useEffect(() => {
    if (mainRef.current != null) {
      let index = -2;
      if (currentPage) {
        const page = pageIndexs.find((p) => p.name === currentPage.name);
        if (page) index = 0 - page.index;
      }
      const tl = gsap.timeline();
      tl.to(mainRef.current, { duration: 0.1, autoAlpha: 0, x: index * width });
    ...
    const end = (event: TouchEvent | MouseEvent) => {
      ...
      if (Math.abs(diffX) > 50) {
        ...
        if (index !== indexRef.current) {
          const page = pageIndexs?.find((p) => p.index === index);
          if (page) {
  ...
  useEffect(() => {
    if (currentPage && initedRef.current) {
