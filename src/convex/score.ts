...
export const findRanks = internalQuery({
  ...
  handler: async (ctx, { tournamentId, numbers }) => {
    const ranks = await ctx.db.query("score")
...
export const find = internalQuery({
  ...
  handler: async (ctx, args) => {
    let board = null;
    const boardItem = await ctx.db.query("score")
...
export const create = internalMutation({
  args: { uid: v.string(), tournamentId: v.string(), points: v.number() },
  handler: async (ctx, args) => {
    // const cells = initGame();
    const boardId = await ctx.db.insert("score", { ...args, lastUpdate: Date.now() });
    return boardId;
  },
});
export const update = internalMutation({
  args: { boardId: v.id("score"), points: v.number() },
