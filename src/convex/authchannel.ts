...
export const create = mutation({
  args: { id: v.number(), authenticator: v.string(), data: v.any() },
  handler: async (ctx, { id, authenticator, data }) => {
    const docId = await ctx.db.insert("authchannel", { id, authenticator, data });
    return docId
  },
});
